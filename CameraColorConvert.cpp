...
#define FIX1P8(x) ((int)((x) * (1<<8)))
     ...
     {
          int h=0;
          ALOGE_IF(DEBUG_COLORCONVERT,"%s: yuv stride = %d, rgbstride = %d , size = %dx%d", __FUNCTION__,pyuvstride, prgbstride, width, height);
     ...
     {
          int h=0;
          ALOGE_IF(DEBUG_COLORCONVERT,"%s: yuv stride = %d, rgbstride = %d , size = %dx%d", __FUNCTION__, pyuvstride, pbgrstride, width, height);
     ...
     {
          ALOGE_IF(DEBUG_COLORCONVERT,"%s: dstStride = %d, dstHeight = %d, srcStride = %d, size = %dx%d", __FUNCTION__,dstStride, dstHeight, srcStride,
                   width, height);
 ...
 */
     ...
     {
          ALOGE_IF(DEBUG_COLORCONVERT,"%s: size = %dx%d", __FUNCTION__, width, height);
          int h = 0;
          int w = 0;
	...
	void CameraColorConvert::convert_yuv420p_to_rgb565(CameraYUVMeta *yuvMeta, uint8_t *dstAttr)
	{
	 ...
	 void CameraColorConvert::yuv420tile_to_yuv420sp(CameraYUVMeta* yuvMeta, uint8_t* dest) {
		  ALOGE_IF(DEBUG_COLORCONVERT,"%s: width = %d, height = %d, format = %d",
