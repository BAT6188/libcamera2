...
namespace android{
	 ...
	 int CameraCompressorHW::hw_compress_to_jpeg() {
		  ...
		  ALOGE_IF(DEBUG_COMPRESS_HW,"jpeg_out address = %p",jpeg_out);
		  ALOGE_IF(DEBUG_COMPRESS_HW,"jpeg_size address = %p",jpeg_size);
		  ALOGE_IF(DEBUG_COMPRESS_HW,"tlb_address = %08x",tlb_addr);
		  ...
		  ALOGE_IF(DEBUG_COMPRESS_HW,"hw_reginfo_address = %p",hw_reginfo);
	 ...
	 int CameraCompressorHW::put_image_jpeg(unsigned char* yuv_y, unsigned char* yuv_c, 
		  ...
		  ALOGE_IF(DEBUG_COMPRESS_HW,"s->des_va = %08x, s->des_pa = %08x",(int)s->des_va, (int)s->des_pa);
		  ...
		  ALOGE_IF(DEBUG_COMPRESS_HW,"s->bsa = %08x, s->p0a = %08x, s->p1a = %08x",
				   (int)s->bsa, (int)s->p0a, (int)s->p1a);
		  ALOGE_IF(DEBUG_COMPRESS_HW,"s->ncol = %d, s->nrsm = %d, s->nmcu = %d, s->ql_sel = %d", 
				   (int)s->ncol, (int)s->nrsm, (int)s->nmcu, (int)s->ql_sel);
		  ALOGE_IF(DEBUG_COMPRESS_HW,"tlb_addr = %08x",tlb_addr);
		  ...
		  *((volatile unsigned int *)(vpu_base + REG_SCH_TLBA)) = tlb_addr;
		  ...
		  ALOGE_IF(DEBUG_COMPRESS_HW,"vpu_base + REG_SCH_TLBA = %08x\n",
		  ...
		  // while(!(read_reg(jpgc_base, 0x8) & 0x80000000)) {

		  // 	   ALOGE_IF(DEBUG_COMPRESS_HW,"REG_JPGC_BSA = %08x\n",
		  // 				(unsigned int)read_reg(jpgc_base, 0x0c));
		  // 	   ALOGE_IF(DEBUG_COMPRESS_HW,"REG_JPGC_P0A = %08x\n",
		  // 				(unsigned int)read_reg(jpgc_base, 0x10));
		  // 	   ALOGE_IF(DEBUG_COMPRESS_HW,"REG_JPGC_P1A = %08x\n",
		  // 				(unsigned int)read_reg(jpgc_base, 0x14));
		  // 	   ALOGE_IF(DEBUG_COMPRESS_HW,"REG_JPGC_NMCU = %08x\n",
		  // 				(unsigned int)read_reg(jpgc_base, 0x28));
		  // 	   ALOGE_IF(DEBUG_COMPRESS_HW,"REG_JPGC_NRSM = %08x\n",
		  // 				(unsigned int)read_reg(jpgc_base, 0x2c));
		  // 	   ALOGE_IF(DEBUG_COMPRESS_HW,"REG_JPGC_P0C = %08x\n",
		  // 				(unsigned int)read_reg(jpgc_base, 0x30));
		  // 	   ALOGE_IF(DEBUG_COMPRESS_HW,"REG_JPGC_P1C = %08x\n",
		  // 				(unsigned int)read_reg(jpgc_base, 0x34));
		  // 	   ALOGE_IF(DEBUG_COMPRESS_HW,"REG_JPGC_P2C = %08x\n",
		  // 				(unsigned int)read_reg(jpgc_base, 0x38));
		  // 	   ALOGE_IF(DEBUG_COMPRESS_HW,"REG_JPGC_GLBI = %08x\n",
		  // 				(unsigned int)read_reg(jpgc_base, 0x04));
		  // 	   ALOGE_IF(DEBUG_COMPRESS_HW,"VDMA task trigger = %08x\n",
		  // 				(unsigned int)read_reg(gp0_base, 0x8));
		  ...
		  *(dest_tmp->pSize)=dest_tmp->nOutOffset;
		  ...
		  ALOGE_IF(DEBUG_COMPRESS_HW,"dest_tmp->outdata = %08x, *(dest_tmp->pSize) = %d ",
		  ...
		  ALOGE_IF(DEBUG_COMPRESS_HW,"end work.......................................");
	 ...
	 void CameraCompressorHW::yuv422i_to_yuv420_block(unsigned char *dsty,unsigned char *dstc,
		  ...
		  ALOGE_IF(DEBUG_COMPRESS_HW,"%s: start doing yuv422i_to_yuv420_block",__FUNCTION__);
		  ...
		  ALOGE_IF(DEBUG_COMPRESS_HW,"  show me w_offset=%d,  h_offset=%d",w_offset,h_offset);
		  ALOGE_IF(DEBUG_COMPRESS_HW,"width = %08x,height= %08x,w= %08x,h= %08x",width,height,w,h);

		  for (y = 0; y < height>>4 ; y++) {
			
			   ALOGE_IF(DEBUG_COMPRESS_HW,"show me y = %d",y);
						 ...
						 for (j=0; j<8; j++) {
							  ...
							  if (i%2) {
								   if(((y*16+i) >= h) || ((x*16+j*2) >= w)) {
										ALOGE_IF(DEBUG_COMPRESS_HW,"out of range");
										*outy++ = 0;
										*outy++ = 0;
								   }
								   else {
	 ...
	 void CameraCompressorHW::camera_mem_free(struct camera_buffer* buf) {
		  ...
		  if(buf->common != NULL) {
			   ALOGE_IF(DEBUG_COMPRESS_HW,"now doing camera_mem_free");
	 ...
	 int CameraCompressorHW::camera_mem_alloc(struct camera_buffer* buf,int size,int nr) {
		  ...
		  ALOGE_IF(DEBUG_COMPRESS_HW,"now doing camera_mem_alloc");
		  ...
		  ALOGE_IF(DEBUG_COMPRESS_HW," get_buffer hou %p",buf->common->data);
