/*
 * Camera HAL for Ingenic android 4.0
 *
...
namespace android {
	 ...
	 CameraCompressor::CameraCompressor(compress_params_t* yuvImage,bool mirror,int rot) {
          ...
          mThumbnailWidth = (yuvImage->thumbnailWidth != 176) ? 176 : (yuvImage->thumbnailWidth);
          mThumbnailHeight = (yuvImage->thumbnailHeight != 144) ? 144 : yuvImage->thumbnailHeight;
	 ...
	 status_t CameraCompressor::compressRawImage(int width, int height, int quality) {
          ...
          ALOGE_IF(DEBUG_COMPRESSOR,"%s: %p[%dx%d]", __FUNCTION__, mSrc, width, height);
          ...
          if ((NULL != mEncoder) && (mEncoder->encode(&mStream, (void*)pY, 
													  width, height, offsets, quality))) {
               ALOGE("%s: Compressed JPEG: %d[%dx%d] -> %d bytes",
